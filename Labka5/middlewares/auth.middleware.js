...
const internsService = require('../services/interns.service');
...
async function signinDataValidation(req, res, next) {
    try {
        ...
        const intern = await internsService.findOne({ email }, {});

        if (!intern) {
            throw createError.NotFound('There is no Intern with such email');
        }

        const passwordCheck = await bcrypt.compare(password, intern.password);
